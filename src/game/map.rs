...
/// State mainly for separating level setup and in level systems
...
/// Specifies the current dungeon section and floor
#[derive(Debug)]
...
/// Resource containing configurations for generating the next floor.
/// 
...
#[derive(Resource)]
pub struct NextFloorConfigs {
    size: MapSize
}

impl NextFloorConfigs {
    /// This is fundamentally how its going to be set up, even after the rest of the complexity is figured out
    /// 
    /// Ranges for the randomized values will vary for different sets of levels
    /// 
    /// (think dcss rock caves having a chance to appear only on level 5+)
    pub fn from_floor(floor: DungeonFloor) -> Result<Self, MapError> {
        let size = match floor {
            DungeonFloor::Dungeon(1..=2) => MapSize::new(49, 75),
            DungeonFloor::Dungeon(3..=5) => MapSize::new(59, 99),
            DungeonFloor::Dungeon(6..=10) => MapSize::new(39, 59),
...
impl Map {
    ...
    pub fn new_from_configs(configs: NextFloorConfigs) -> Self {
        let count = configs.size.count();
...
impl From<TileKind> for Tile {
    fn from(value: TileKind) -> Self {
        Self {kind: value, visible: false, discovered: false }
    }
}
